...
def score_rfm(df: pd.DataFrame, cfg: RfmConfig = RfmConfig()) -> pd.DataFrame:
	...
	out["R"] = pd.qcut(-out["recency_days"], cfg.recency_bins, labels=False, duplicates="drop") + 1
	out["F"] = pd.qcut(out["frequency"], cfg.frequency_bins, labels=False, duplicates="drop") + 1
	out["M"] = pd.qcut(out["monetary_value"], cfg.monetary_bins, labels=False, duplicates="drop") + 1
