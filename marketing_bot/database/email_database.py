...
class EmailDatabase:
    ...
    def _init_files(self) -> None:
        ...
        if not self.contacts_file.exists():
            contacts_df = pd.DataFrame(columns=[
                'email', 'name', 'segment', 'customer_id', 
        ...
        if not self.campaigns_file.exists():
            campaigns_df = pd.DataFrame(columns=[
                'campaign_id', 'name', 'subject', 'body', 'segment', 
                'created_at', 'sent_count', 'success_count'
            ])
    ...
    def add_contacts_from_csv(self, csv_file: Path, segment: Optional[str] = None) -> int:
        ...
        try:
            ...
            existing_df = pd.read_csv(self.contacts_file) if self.contacts_file.exists() else pd.DataFrame()
            ...
            added_count = len(df) - len(existing_df) if not existing_df.empty else len(df)
    ...
    def save_campaign(self, campaign_id: str, name: str, subject: str, body: str, 
                     segment: str, sent_count: int = 0, success_count: int = 0) -> None:
