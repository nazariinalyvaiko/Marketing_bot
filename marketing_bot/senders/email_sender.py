...
def send_email(msg: EmailMessage) -> None:
	...
	dry_run = settings.SENDER_DRY_RUN or os.getenv("SENDER_DRY_RUN", "true").lower() == "true"
	from_name = msg.from_name or settings.EMAIL_SENDER_NAME
	from_email = msg.from_email or settings.EMAIL_SENDER_ADDR
	
	if dry_run:
		logger.info(f"[dry-run] Email to={msg.to} from={from_name} <{from_email}>\nSubject: {msg.subject}\n\n{msg.body}")
		return

	# Try SendGrid first
	if SENDGRID_AVAILABLE and settings.SENDGRID_API_KEY and settings.SENDGRID_FROM_EMAIL:
	...
	logger.warning("No email provider configured. Set SENDER_DRY_RUN=true or provide SENDGRID_API_KEY or SMTP_ env vars.")


def _sendgrid_send(from_email: str, from_name: str, to_email: str, subject: str, body: str) -> None:
...
def _smtp_send(from_email: str, from_name: str, to_email: str, subject: str, body: str) -> None:
