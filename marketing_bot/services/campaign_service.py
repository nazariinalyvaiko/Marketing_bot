...
from marketing_bot.models.campaign import Campaign, CampaignResult, CampaignStatus, CampaignType
...
class CampaignService:
    def __init__(self, campaign_repo: CampaignRepository, metrics_tracker: MetricsTracker):
    ...
    async def execute_campaign(self, campaign_id: UUID, customer_data: List[dict]) -> List[CampaignResult]:
        ...
        logger.info(f"Executing campaign {campaign.name} for {len(customer_data)} customers")
        ...
        for _, customer in scored_customers.iterrows():
            try:
                ...
                logger.error(f"Failed to process customer {customer['customer_id']}: {e}")
                await self.metrics_tracker.track_campaign_execution(
                    campaign_id=campaign_id,
                    customer_id=customer['customer_id'],
                    success=False,
    ...
    async def _process_customer(self, campaign: Campaign, customer: dict) -> List[CampaignResult]:
