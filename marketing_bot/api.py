...
@app.get("/metrics")
def get_metrics(metrics: MetricsTracker = Depends(get_metrics_tracker)) -> Dict[str, Any]:
    """Get system metrics."""
...
async def create_campaign(
    ...
    try:
        ...
        return {"campaign": result.model_dump(), "message": "Campaign created successfully"}
...
def generate(req: GenerateRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        if req.kind in ("email", "both"):
            ...
            email_content = generate_marketing_text(email_prompt, model=req.model, max_tokens=req.max_tokens)
            
        if req.kind in ("social", "both"):
            social_prompt = render_prompt(SOCIAL_POST_TEMPLATE, ctx)
            social_content = generate_marketing_text(social_prompt, model=req.model, max_tokens=req.max_tokens)
