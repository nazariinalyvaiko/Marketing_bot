...
@cli.command()
@click.option("--customers-csv", type=click.Path(exists=True, path_type=Path), default=Path("data/customers.csv"))
@click.option("--top-n", type=int, default=5, help="Top N customers per segment to preview")
...
@click.option("--tone", type=click.Choice(["friendly", "professional", "playful", "urgent"]), default="professional")
...
def generate(segment_name: str, product_name: str, goal: str, offer: str, tone: str, to_email: str, platform: str, preview: bool, kind: str, model: Optional[str], max_tokens: int) -> None:
	...
	if kind in ("email", "both"):
		...
		email_content = generate_marketing_text(email_prompt, model=model, max_tokens=max_tokens)
	...
	if kind in ("social", "both"):
		...
		social_content = generate_marketing_text(social_prompt, model=model, max_tokens=max_tokens)
		if preview:
